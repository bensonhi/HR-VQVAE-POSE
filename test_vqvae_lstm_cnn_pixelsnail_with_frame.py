import sys
sys.path.append('..')
from video.dataset import MnistVideoCodeLMDBDataset2
from video.dataloader import video_mnist_dataloader
from torchvision import utils
import numpy as np
from video.LSTM3 import LSTM3
# from video.train_vqvae_lstm_cnn_pixelsnail2_with_lastframe import train
from video.train_vqvae_lstm_cnn_pixelsnail2_with_lastframe_add_noise import train
...
def callback(samples ,frames , epoch, _iter):
    ...
    with torch.no_grad():
        ...
        path = 'output_samples/{}/vqvae_lstm_pixelsnail_epoch{}_iter{}.png'.format(*[run_num,epoch,_iter])
...
def train_model(checkpoint):
    ...
    dataset = MnistVideoCodeLMDBDataset2(lambda_name, frame_len )
    loader = video_mnist_dataloader(dataset, batch_size, shuffle=True,num_workers=4)
    ...
    train(checkpoint, lstm_model, pixel_model,input_channel, loader, callback, epoch_num, device, lr, run_num,num_frame_learn=num_frame_learn, image_samples=image_samples)
